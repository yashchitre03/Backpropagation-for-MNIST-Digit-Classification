...
train_std = np.std(train_X[0:train_size].flatten())
test_mean = np.mean(test_X[0:test_size].flatten())
test_std = np.std(test_X[0:test_size].flatten())
...
while True:
    ...
    for i in range(train_size):
        ...
        x = (train_X[i].flatten() - train_mean) / train_std
    ...
    for j in range(test_size):
        ...
        x = (test_X[j].flatten() - test_mean) / test_std
    ...
    if train_energy_list[epoch] > train_energy_list[epoch-1]:
        eta *= 0.9
    if test_error_list[epoch]/test_size < 0.05:
        break
    epoch += 1   
    if epoch == 1001:
        break
...
plt.plot(range(len(train_error_list)), train_error_list, 'b.', label='Training Data')
plt.plot(range(len(test_error_list)), test_error_list, 'r.', label='Testing Data')
...
plt.plot(range(len(train_energy_list)), train_energy_list, 'b.', label='Training Data')
